...
class PQueue
{
    ...
    public:
        ...
        Node(Cust *data, int priority, Node* next) : data(data), priority(priority), next(next) {}
    };
    ...
    PQueue() : head(nullptr) {}
     ~PQueue();   // Destructor deallocates memory used by the nodes in the priority queue
